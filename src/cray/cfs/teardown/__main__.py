...
def _get_targets(cfs_name: str) -> Tuple[Iterable, Iterable]:
...
def _finish_the_job(job_id: str, cfs_name: str) -> None:
    ...
    for ssh_container in response['ssh_containers']:
        if ssh_container['name'] == cfs_name:
            ...
            key = paramiko.RSAKey.from_private_key_file('/secret/key')
...
def _update_cfs_with_result(
        cfs_name: str, cfs_namespace: str, image_id: str, result_image_id: str
        ) -> None:
    """
    ...
    cfs_obj = cfs_api.get(cfs_name, cfs_namespace)
    existing_artifacts = cfs_obj['spec']['status']['artifacts']
    ...
    body = {
        "spec": {"status": {"artifacts": existing_artifacts}}
    }
    cfs_api.patch(cfs_name, body, namespace=cfs_namespace)
    return
...
def main() -> None:  # noqa: C901
    ...
    try:
        failed, success = _get_targets(cfs_name)
    except Exception:
    ...
    try:
        while all_image_ids:
            ...
            # Ansible run and teardown for this image
                ...
                try:
                    _update_cfs_with_result(cfs_name, cfs_namespace, image_id, response)
                except Exception as err:
                    teardown_success = False
                    LOGGER.error(
