...
class ImageRootInventory(CFSInventoryBase):
    ...
    def _upload_public_key(cfs_session: str, namespace: str) -> str:
        ...
        # Create Public Key
        host, port, session = get_IMS_API()
        key = ImageRootInventory._read_ssh_public_key(namespace)
    ...
    @staticmethod
    def _read_ssh_public_key(cfs_namespace: str) -> str:
        """
        Configuration Framework Service public key is stored as a configmap; read it
        and return it.
        """
        try:
            public_key = k8score.read_namespaced_config_map(
                'cray-cfs-aee-pubkey', cfs_namespace
            )
        except ApiException as err:
            raise CFSInventoryError(
                "SSH Public key information not available via configmap 'cray-cfs-aee-pubkey'. "
                "Ensure that configmap 'cray-cfs-aee-pubkey' exists. "
                "Error: %s" % err
            )
        return public_key.data['key'].strip()
